...
log.init(prefix='document_loader_qdrant')
...
class SingleDocumentProcess(Process):
    ...
    def process_file(self, doc_file: DocFile):
        ...
        self.qdrant_ops.save_to_qdrant(self.emb_obj.get_by_name(TRANSFORMER_MODEL), self.collection_name, split_docs)

    def run(self):
        log.init(prefix=f'document_loader_qdrant_{self.process_index}')
