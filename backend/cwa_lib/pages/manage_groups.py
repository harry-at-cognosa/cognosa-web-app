...
class ManageGroupsTable:
    ...
    async def query_all(
        ...
        return ManageGroupsQueryResult(
            name='manage_groups',
            rows=rows,
            columns=manage_groups__query_columns,
            table_options=manage_groups__table_options,
            order_by=order_by,
            order_dir=order_dir,
            total=len(rows)
        )
    
    async def create_one(self, data: ManageGroupsCreate) -> TableCreateRowResult:
        """
        Create one row
        """
    ...
    async def update_one(self, data: ManageGroupsUpdate) -> TableUpdateRowResult:
        """
        Update one row
        """
        update_values = dict()
