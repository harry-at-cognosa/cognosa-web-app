...
gvdbs_edit_columns = [x for x in manage_vdbs__query_columns.keys() if (x not in ('gvdbs_id', 'gvdbs_status'))]

manage_vdbs__table_options = TableOptions(
    title='Group VDBs',
    pk='gvdbs_id',
...
class ManageVDBsTable:
    ...
    async def query_all(
            ...
            payload: TableQuery,
            deleted: int | None
            ) -> ManageVDBsQueryResult:
        
        manage_vdbs__qc = deepcopy(manage_vdbs__query_columns)
        manage_vdbs__to = deepcopy(manage_vdbs__table_options)
    ...
    async def create_one(self, data: ManageVDBsCreate) -> TableCreateRowResult:
        """
        Create one row
        """
        await fix_autoincrement(self.session, GroupVDBs)
        defaults = {col: manage_vdbs__query_columns[col].default 
    ...
    async def update_one(self, data: ManageVDBsUpdate) -> TableUpdateRowResult:
        """
        Update one row
        """
